...
|```extreme_rainfall_interpolation_method```| The interpolation method to use for extreme rainfall depths. There are three options: **1)** The parabolic method provided in ARR is ```SiriwardenaWeinmann1998```; or **2)** a variation of this, using a normal scale for the AEP axis, is ```HillAndOthers2000```; **3)** It is recommended that ```GEV``` is used as this will deal with vertex perplexity that sometimes arises with the parabolic methods.|
 |```extreme_spatial_smoothing_method```| The interpolation method to use for extreme rainfall spatial patterns. The ```interpolate_weights``` tag interpolates spatial scaling in the ```aep_changeover_to_extreme``` zone (defined in the storm config file). On either side of this zone, ARR and GSDM/GTSMR spatial patterns are applied. Defaults to ```interpolate_depths``` method, which essentially applies the GSDM and GTSMR spatial patterns to the PMP only, and spatial patterns are interpolated between 1 in 2,000 and PMP.
|```pmp_exceedance_sampling```|(Optional) This key is unlikely to be required. It provides the user an option to deviate from the standard sampling method whereby rainfall for AEPs rarer than the AEP of the PMP is extrapolated from the curve fitted to the extreme rainfalls (```extrapolate```). While not recommended, the rainfall sampled beyond the AEP of the PMP can be capped to match the PMP depth by setting this key to: ```cap```.|
