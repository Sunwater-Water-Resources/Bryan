...
While a batch is running, the console window title shows which simulation is in progress - ```Bryan 3/21: <output name> - <sims list>``` - and a summary of how the batch finished once it ends, so a long run can be followed from the taskbar without reading the scroll. The same information is printed as a header above each simulation, which is what you see when the output is redirected to a file.
