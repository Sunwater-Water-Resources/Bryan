...
- The **storage table** (```.els```: ```EL, A, V```). Rows of no storage at the bottom of the table, below the lowest outlet or where the survey starts, are left out: the volume curve starts at the last of them.
- The **ratings** in force over the record: a **register** (an xlsx: a ```Register``` sheet of ```Rating, from, to, FSL, ...``` and one ```level, flow``` sheet per rating), or, for a dam whose spillway has not changed, **one rating** for the whole record - a URBS ```.rat``` (level and flow) or ```.sq```, or a ```level,flow``` csv. One rating's full supply level comes from its file where it says (```FSL is 265.8 mAHD``` in the header, or a level table's first row); otherwise give it in the box that appears beside it, as a ```.sq``` without one in its header needs. With one rating, homogenising routes the record through much the rating it was made under, so the homogenised levels follow the recorded ones; what it is for is the antecedent storage and the inflow record.
