...
```
Any number of dam paths can be appended to the dictionary. Then the path is applied in the ```CollateIFDtable.py``` script as follows, where the ```subcatchmentcolumn``` specifies the header name in the shapefile with the subcatchment names (values must be integers like used in URBS):
```python
