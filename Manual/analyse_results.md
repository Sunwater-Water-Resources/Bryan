# Result analysis
Once the simulations have been run, the results can be analysed setting the ```Analyse results``` key in the [simulation list](SubDocs/sim_list.md) to *yes*. At this point you may want to set the ```Run models``` key in the [simulation list](SubDocs/sim_list.md) to *no* to avoid rerunning the simulations. 
