...
class StormBurst:
    ...
    def skip_extreme_methods(self, duration_range, do_preburst=False, do_filtering=False):
        ...
        if self.rainfall.extreme_spatial_method == 'interpolate_depths':
            if do_preburst:
                ...
                if len(duration_range) == 1:  # Monte Carlo - single duration
                    if duration_range[0] == max(self.rainfall.durations):
                        shortest_duration_to_load = sorted(self.rainfall.durations)[
                            -2]  # load 2nd largest duration to allow envelope to be extrapolated

                for dur in self.rainfall.durations:
                    if dur >= shortest_duration_to_load:
                        self.preburst_envelope_durations.append(dur)
            if do_filtering:
                shortest_duration_to_load = min(self.rainfall.durations)
    ...
    def set_up_extreme_rainfall(self, duration_range):
        duration_changeover = self.storm_method_config['gsdm_gtsmr_changover_duration']
        self.rainfall.skip_method = self.skip_method
        print('\nSetting up the extreme rainfall')
        self.rainfall.set_up_extreme_rain(duration_changeover, duration_range, self.subcatch_area)
    ...
    def filter_embedded_bursts_in_preburst(self, preburst_pattern0, main_burst, main_burst_duration,
                                           ...
                                           climate_adjustment = None, cap_duration = True, method=None):
