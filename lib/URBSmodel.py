...
class UrbsModel:
    ...
    def find_dam_routing_line(self, dam_location):
        ...
        return {"line": None, "line_number": None}
    ...
    def write_batch_file(self, batch_path, header, urbs_exe, vec_path, storm_path,
                         result_name, parameter_string, retries=1, delay=1.0):
        ...
        for attempt in range(retries + 1):
            try:
                with open(batch_path, 'w', encoding='utf-8') as f:
                    f.writelines(txt + '\n' for txt in header)
                    f.write(
                        f'"{urbs_exe}" {vec_path} {storm_path} '
                        f'{result_name} {parameter_string}\n'
                    )
                return  # success
            except PermissionError:
                if attempt >= retries:
                    raise  # fail after retry
                time.sleep(delay)
    ...
    def run_storm(self, storm_name, result_name):
        ...
        # Changed the above to a function that handles permission conflicts due to unclosed file
